...
def CreateExternal_GetConfidence(DealJobs_instance, project_dir, source_particle_job, source_particle_parameter_name, if_slurm, if_initial_orientation_balance, accelerate_port_list, accelerate_gpu_ids):
        ...
        if if_need_run:
            if if_slurm:
                ...
                with open(globaldir + 'metadata/GetConfidence.sh', 'a') as f:
                    ...
                    if accelerate_gpu_ids is not None:
                        f.write('export CUDA_VISIBLE_DEVICES=\'' + accelerate_gpu_ids + '\'\n')
                    f.write('cd ' + globaldir + '\n')
                    f.write('srun accelerate launch  --mixed_precision=bf16 ' + filedir + 'get_scores.py\n')
                ...
                if os.path.exists(globaldir + 'metadata/GetConfidence.sh'):
                    os.remove(globaldir + 'metadata/GetConfidence.sh')
                usable_port = mytoolbox.scanusableport(accelerate_port_list)
                print('Accelerate port:', usable_port, flush=True)
                with open(globaldir + 'metadata/GetConfidence.sh', 'w') as f:
                    ...
                    if accelerate_gpu_ids is not None:
                        f.write('export CUDA_VISIBLE_DEVICES=\'' + accelerate_gpu_ids + '\'\n')
                    f.write('cd ' + globaldir + '\n')
                    f.write('accelerate launch  --mixed_precision=bf16 --main_process_port=' + (str)(usable_port) + ' ' + filedir + 'get_scores.py\n')
                if not os.path.exists(globaldir + 'metadata/processpids'):
                    os.makedirs(globaldir + 'metadata/processpids')
...
def CreateExternal_PlotConfidenceCurve(DealJobs_instance, project_dir, source_nurefine_job, source_nurefine_parameter_name, curvedata, confidence_dict):
    # DealJobs_instance: 实例化的DealJobs，使用自己的账号密码实例化
...
def SelectAndRefine(project_dir):
    ...
    accelerate_port_list = [_ for _ in range(parameters['accelerate_port_start'], parameters['accelerate_port_end'] + 1)]
    accelerate_gpu_ids = parameters['inference_gpu_ids']
    ...
    if if_initial_orientation_balance:
        ...
        orientation_diagnostics_parameters = mytoolbox.readjson(
            globaldir + 'parameters/orientation_diagnostics_parameters.json')
    ...
    new_external_get_confidence_job = CreateExternal_GetConfidence(dealjobs, globaldir, source_particle_job, source_particle_parameter_name, if_slurm, if_initial_orientation_balance, accelerate_port_list, accelerate_gpu_ids)
    ...
    for key, _ in nurefine_jobs_parents.items():
        ...
        if if_initial_orientation_balance:
            ...
            new_orientation_job = orientation_diagnostics_class.QueueOrientationDiagnosticsJob(
                orientation_diagnostics_parameters, key, 'volume', [key], ['particles'])
            if not if_safe_mode:
    ...
    new_external_plot_confidence_curve_job = CreateExternal_PlotConfidenceCurve(dealjobs, globaldir, best_final_nurefine_job_uid, 'particles', whole_nurefine_jobs_resolutions_curvedata, confidence_dict)
